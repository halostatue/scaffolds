...
Contribution to {{ .ProjectPascal }} is encouraged: bug reports, discussions,
...
- I use code formatters and linting to ensure consistent styles and formatting.
  There should be no warnings output from compile or test run processes. I use
...
{{ .ProjectPascal }} contributions must be well understood by the submitter
...
{{ .ProjectPascal }} has adopted a variation of the Conventional Commits
...
{{ .Computed.author }} created {{ .ProjectPascal }}.
