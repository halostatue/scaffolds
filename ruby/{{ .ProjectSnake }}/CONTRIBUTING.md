...
Contribution to {{ .ProjectPascal }} is encouraged: bug reports, feature
...
{{ .ProjectPascal }} is governed under the
...
- Versions must not be updated in pull requests unless otherwise directed. This
  ...
  - Modify `VERSION` in `lib/{{ .ProjectSnake }}/version.rb`. When your patch is
    accepted and a release is made, the version will be updated at that point.

  - Modify `{{ .ProjectSnake }}.gemspec`; it is a generated file. (You _may_ use
...
- Documentation should be added or updated as appropriate for new or updated
  functionality. The documentation is RDoc; {{ .ProjectPascal }} does not use
...
{{ .ProjectPascal }} contributions must be well understood by the submitter and
...
{{ .ProjectPascal }} uses Ryan Davis's [Hoe][Hoe] to manage the release process,
and it adds a number of rake tasks. You will mostly be interested in `rake`,
which runs the tests the same way that `rake test` will do.

To assist with the installation of the development dependencies for {{
.ProjectPascal }}, I have provided the simplest possible Gemfile pointing to the
(generated) `{{ .ProjectSnake }}.gemspec` file. This will permit you to do
...
{{ .ProjectPascal }} has adopted a variation of the Conventional Commits format
